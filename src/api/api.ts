...
/** API サーバのホスト名（とポート番号） */
const API_SERVER = "http://api.kohga.local";
...
export async function createUser() {
  ...
  const res = await fetch(API_SERVER + API_PATH, {
    method: "GET",
  });
 ...
 * @throws Fetch API に由来する例外
 */
export async function reqRoute(
  userId: ApiTypes.UserId,
  args: ApiTypes.ReqRouteArg
) {
 ...
 * @throws Fetch API に由来する例外
 */
export async function execRoute(
  userId: ApiTypes.UserId,
